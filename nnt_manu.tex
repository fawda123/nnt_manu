...
\hlcom{# data preprocessing}
\hlcom{# select UA carrior for month of december and relevant variables}
\hlcom{# normalize and standardize variables}
\hlstd{tomod} \hlkwb{<-} \hlkwd{filter}\hlstd{(flights, month} \hlopt{==} \hlnum{12} \hlopt{&} \hlstd{carrier} \hlopt{==} \hlstr{'UA'}\hlstd{)} \hlopt{%>%}
  \hlkwd{select}\hlstd{(arr_delay, dep_delay, dep_time, arr_time, air_time, distance)} \hlopt{%>%}
  \hlkwd{mutate_each}\hlstd{(}\hlkwd{funs}\hlstd{(scale),} \hlopt{-}\hlstd{arr_delay)} \hlopt{%>%}
  \hlkwd{mutate_each}\hlstd{(}\hlkwd{funs}\hlstd{(as.numeric),} \hlopt{-}\hlstd{arr_delay)} \hlopt{%>%}
  \hlkwd{mutate}\hlstd{(}\hlkwc{arr_delay} \hlstd{= scales}\hlopt{::}\hlkwd{rescale}\hlstd{(arr_delay,} \hlkwc{to} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{1}\hlstd{)))} \hlopt{%>%}
  \hlstd{data.frame}

\hlcom{# create two models}
\hlcom{# random starting weights are different for each}
\hlstd{ex1} \hlkwb{<-} \hlkwd{nnet}\hlstd{(arr_delay} \hlopt{~} \hlstd{.,} \hlkwc{size} \hlstd{=} \hlnum{5}\hlstd{,} \hlkwc{linout} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{data} \hlstd{= tomod)}
\hlstd{ex2} \hlkwb{<-} \hlkwd{nnet}\hlstd{(arr_delay} \hlopt{~} \hlstd{.,} \hlkwc{size} \hlstd{=} \hlnum{5}\hlstd{,} \hlkwc{linout} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{data} \hlstd{= tomod)}

\hlcom{# view olden results }
\hlkwd{olden}\hlstd{(ex1)}
\hlkwd{olden}\hlstd{(ex2)}
\end{alltt}
\end{kframe}
