...
tots <- c(
  ...
  sum(cran_downloads('nonmlp', from = '2000-01-01', to = today)$count),
  sum(cran_downloads('qrnn', from = '2000-01-01', to = today)$count),
  sum(cran_downloads('frbf', from = '2000-01-01', to = today)$count)
)

@

See refs in the RSNNS paper!!

Methods in \pkg{NeuralNetTools} were written for several \proglang{R} packages that can be used to create \ac{mlp} neural networks: \pkg{neuralnet} \citep{Fritsch12}, \pkg{nnet} \citep{Venables02}, and \pkg{RSNNS} \citep{Bergmeir12}. The \code{train} function from the \pkg{caret} package \citep{Kuhn15} can be used to create a neural network model by repeated training with functions from the remaining packages.  Additional packages without \pkg{NeuralNetTools} methods include....

An additional package, \pkg{FCNN4R} \citep{Klima15}, provides an interface between \pkg{R} and the \pkg{FCNN} \proglang{C++} library.  At the time of writing, \pkg{FCNN4R} is a new package with less than 1\% of the total downloads of all neural network packages on CRAN.  As such, \pkg{NeuralNetTools} does not include methods for the \pkg{FCNN4R} package, although future versions may do so depending on the popularity of \pkg{FCNN4R}.  In general, the remaining packages use similar methods to create \ac{mlp} networks such that the resulting models are comparable.  Differences between the packages primarily include default values of the arguments and the ability to include multiple response variables or multiple hidden layers.  The primary functions in each package also vary in the type of input used to specify the model structure.  Differences that influence how the \pkg{NeuralNetTools} functions handle object classes from the different packages are described below. 
...
Tbe \code{plotnet} function plots a neural network as a neural interpretation diagram \citep{Ozesmi99} and includes several options to customize plot aesthetics. A neural interpration diagram is a modification to the standard conceptual illustration of the \ac{mlp} network that allows the user to change the thickness and color of the weight connections based on mangnitude and sign, respectively.  The default settings plot positive weights between layers as black lines and negative weights as grey lines. Line thickness is in proportion to absolte magnitude of each weight. The first layer includes only input variables with nodes labelled arbitrarily as I1 through In for n input variables. One through many hidden layers are plotted with each node in each layer labelled as H1 through Hn. The output layer is plotted last with nodes labeled as O1 through On. Bias nodes connected to the hidden and output layers are also shown. Differences in the behavior of \code{plotnet} for different object classes are minimal, with the exception of \code{mlp} class networks that do not include bias layers.  The following shows some examples of basic plots 

<<echo = F>>=
library(NeuralNetTools)
@
<<>>=
# create a model using the nnet package
# neuraldat is included with NeuralDat
@
