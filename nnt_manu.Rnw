...
<<plotprune, eval = T, echo = T, fig.show = 'hold', fig.height = 7, fig.width = 7.7, out.width = '0.48\\textwidth', fig.cap = 'A pruned neural network using the ``Optimal Brain Surgeon\'\' algorithm described in \\citet{Zell98}.  The default plotting behavior of \\code{plotnet} is to omit pruned connections (left), whereas they can be viewed as dashed lines by including the \\code{prune\\_col} argument (right).'>>=
...
The primary benefit of visualizing a neural network interpretation diagram is the ability to evaluate network archicture and the variation in weighted connections between the layers.  For most networks, 
