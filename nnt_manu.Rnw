...
@
<<results = 'hide'>>=
# set random seed for initial inputs during training
set.seed(123)
...
The \pkg{RSNNS} package provides several algorithms that can be used to prune connections in a neural network.  This approach is useful to identify and remove connection weights between layers that do contribute to the predictive performance of the network, which allows for a potential increase in power by increasing the degrees of freedom.  The \pkg{RSNNS} package implements 
