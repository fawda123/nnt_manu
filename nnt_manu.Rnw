...
tots <- c(
  sum(cran_downloads('nnet', from = '2000-01-01', to = '2015-09-13')$count),
  sum(cran_downloads('neuralnet', from = '2000-01-01', to = '2015-09-13')$count),
  sum(cran_downloads('RSNNS', from = '2000-01-01', to = '2015-09-13')$count),
  sum(cran_downloads('FCNN4R', from = '2000-01-01', to = '2015-09-13')$count)
)
@

Packages available in \proglang{R} to create \ac{mlp} neural networks include \pkg{neuralnet}, \pkg{nnet}, and \pkg{RSNNS}. An additional package, \pkg{FCNN4R}, provides an interface between \pkg{R} and the \pkg{FCNN} \proglang{C++} library.  At the time of writing, \pkg{FCNN4R} is a relatively new package that represents less than 1\% of the total downloads of all neural network packages on CRAN.  As such, \pkg{NeuralNetTools} does not include methods for the \pkg{FCNN4R} package, although future versions may do so depending on the popularity of \pkg{FCNN4R}.  In general, the remaining packages use similar methods to create \ac{mlp} networks such that the resulting models should be comparable.  Differences between the packages primarily include default values of the arguments and the ability to handle multiple response variables or multiple hidden layers in the same model.  The \code{nnet} function can take separate (or combined) input or response inputs as each as a separate \code{data.frame} or as a \code{formula}, the \code{neuralnet} function can only use a \code{formula} as input, and the \code{mlp} function (\pkg{RSNNS}) can only take a \code{data.frame} as combined or separate variables as input.  The \code{neuralnet} function is not capable of modelling multiple response variables, unless the response is a categorical variable where one node for each outcome is used in the response.  Additionally, the default output for the \code{neuralnet} function is linear, whereas the opposite is true for the other two functions.  Other differences that influence how functions in \pkg{NeuralNetTools} handle object classes from the different packages are described below. 
