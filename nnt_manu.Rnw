...
# data preprocessing
# select UA carrior for month of december and relevant variables
...
# create two models
# random starting weights are different for each
ex1 <- nnet(arr_delay ~ ., size = 5, linout = TRUE, data = tomod)  
ex2 <- nnet(arr_delay ~ ., size = 5, linout = TRUE, data = tomod)  
