...
<<plotprune, eval = T, echo = T, fig.show = 'hold', fig.height = 7, fig.width = 7.7, out.width = '0.48\\textwidth', fig.cap = 'A pruned neural network using the ``Optimal Brain Surgeon\'\' algorithm described in \\citet{Zell98}.  The default plotting behavior of \\code{plotnet} is to omit pruned connections (\\ref{fig:plotprune1}), whereas they can be viewed as dashed lines by including the \\code{prune\\_col} argument (\\ref{fig:plotprune2}).', fig.subcap = c('', '')>>=
